...
pub unsafe fn special_s_exec(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if step == *FIGHTER_FOX_ILLUSION_STEP_START {
        ...
        if fighter.global_table[PAD_FLAG].get_i32() & *FIGHTER_PAD_FLAG_SPECIAL_TRIGGER != 0 {
            VarModule::on_flag(fighter.battle_object, vars::fox::status::ILLUSION_SHORTEN);
        ...
        && !VarModule::is_flag(fighter.battle_object, vars::fox::status::ILLUSION_SHORTEN) {
            return 0.into();
        }
...
pub unsafe extern "C" fn special_s_handle_step(fighter: &mut L2CFighterCommon) {
    ...
    if step == *FIGHTER_FOX_ILLUSION_STEP_START {
        ...
        if situation != *SITUATION_KIND_AIR {
            ...
            let illusion_end_air_brake_x = if !VarModule::is_flag(fighter.battle_object, vars::fox::status::ILLUSION_SHORTEN) {
            ...
            let end_speed = if !VarModule::is_flag(fighter.battle_object, vars::fox::status::ILLUSION_SHORTEN) {
