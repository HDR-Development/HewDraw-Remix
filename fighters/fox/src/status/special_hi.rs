...
pub unsafe extern "C" fn special_hi_rush_end_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    let ret = smashline::original_status(Main, fighter, *FIGHTER_FOX_STATUS_KIND_SPECIAL_HI_RUSH_END)(fighter);
...
pub unsafe extern "C" fn special_hi_bound_end(fighter: &mut L2CFighterCommon) -> L2CValue {
    let ret = smashline::original_status(End, fighter, *FIGHTER_FOX_STATUS_KIND_SPECIAL_HI_BOUND)(fighter);
...
pub fn install(agent: &mut Agent) {
    ...
    agent.status(Pre, *FIGHTER_FOX_STATUS_KIND_SPECIAL_HI_RUSH_END, special_hi_rush_end_pre);
    agent.status(Main, *FIGHTER_FOX_STATUS_KIND_SPECIAL_HI_RUSH_END, special_hi_rush_end_main);

    agent.status(End, *FIGHTER_FOX_STATUS_KIND_SPECIAL_HI_BOUND, special_hi_bound_end);
}