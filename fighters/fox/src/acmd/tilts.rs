...
unsafe fn game_attackhi3(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        ...
        ATK_POWER(fighter, 2, 7);
        ATK_POWER(fighter, 3, 7);
    }
    wait(lua_state, 2.0);
    if is_excute(fighter) {
        AttackModule::clear_all(boma);
