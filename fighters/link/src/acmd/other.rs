...
unsafe extern "C" fn sound_damagefly(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if !StopModule::is_stop(agent.module_accessor) {
            ...
            if play_vc == 0 {PLAY_FLY_VOICE(agent, Hash40::new("seq_link_rnd_futtobi01"), Hash40::new("seq_link_rnd_futtobi02"));}
        }
    }
    frame(lua_state, 1.1);
    if is_excute(agent) {
...
unsafe extern "C" fn sound_dash(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(agent) {
        let dash_sfx_handle = SoundModule::play_se(agent.module_accessor, Hash40::new("se_link_dash_start"), true, false, false, false, app::enSEType(0));
