...
#[status_script(agent = "falco", status = FIGHTER_STATUS_KIND_SPECIAL_S, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN)]
pub unsafe fn special_s_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    WorkModule::set_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STEP_START, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
    WorkModule::set_int(fighter.module_accessor, -1, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP_PREV);
    ...
    if !StopModule::is_stop(fighter.module_accessor) {
        WorkModule::dec_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
    }
...
pub unsafe extern "C" fn special_s_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let step = WorkModule::get_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
    let situation = fighter.global_table[SITUATION_KIND].get_i32();
    let mut cont = false;
    if !StatusModule::is_changing(fighter.module_accessor) {
        let is_end = MotionModule::is_end(fighter.module_accessor);
        if fighter.global_table[PREV_SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
            if step == *FIGHTER_FALCO_ILLUSION_STEP_FORCE_END {
                WorkModule::set_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STEP_END, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
                cont = true;
            }
            ...
            || step == *FIGHTER_FALCO_ILLUSION_STEP_START {
                if situation == *SITUATION_KIND_GROUND
                || is_end {
                    cont = true;
                }
    ...
    if cont {
        if !StatusModule::is_changing(fighter.module_accessor) {
            ...
            && !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_HIT_SHIELD_TO_END) {
                if MotionModule::is_end(fighter.module_accessor) {
                    WorkModule::inc_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
                    WorkModule::off_flag(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
                }
            }
        }
        ...
        if situation != *SITUATION_KIND_GROUND {
            ...
            if step == *FIGHTER_FALCO_ILLUSION_STEP_END {
                ...
                GroundModule::select_cliff_hangdata(fighter.module_accessor, *FIGHTER_FALCO_CLIFF_HANG_DATA_SPECIAL_S as u32);
                special_s_change_mot(fighter, Hash40::new("special_air_s"));
                fighter.sub_fighter_cliff_check(GROUND_CLIFF_CHECK_KIND_ALWAYS.into());
            }
            else if step == *FIGHTER_FALCO_ILLUSION_STEP_START {
                special_s_air_mot(fighter);
            }
        }
        else {
...
pub unsafe extern "C" fn special_s_change_mot(fighter: &mut L2CFighterCommon, mot: Hash40) {
    if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE) {
        MotionModule::change_motion(
            fighter.module_accessor,
            mot,
            0.0,
        ...
        WorkModule::on_flag(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
    }
    else {
...
#[status_script(agent = "falco", status = FIGHTER_STATUS_KIND_SPECIAL_S, condition = LUA_SCRIPT_STATUS_FUNC_EXEC_STATUS)]
pub unsafe fn special_s_exec(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    set(fighter.module_accessor, *FIGHTER_FALCO_INSTANCE_WORK_ID_FLAG_ILLUSION_LANDING);
    let step = WorkModule::get_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
    let prev_step = WorkModule::get_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP_PREV);
    if step != prev_step {
        special_s_handle_step(fighter);
    }
    if step == *FIGHTER_FALCO_ILLUSION_STEP_START {
        if situation == *SITUATION_KIND_AIR {
            let stop_y_frame = WorkModule::get_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
            if stop_y_frame == 0 {
        ...
        WorkModule::on_flag(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
        special_s_handle_step(fighter);
        WorkModule::set_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STEP_FORCE_END, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
    }
    else if step == *FIGHTER_FALCO_ILLUSION_STEP_RUSH {
        ...
        special_s_handle_step(fighter);
        WorkModule::set_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STEP_FORCE_END, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_HIT_SHIELD) {
            WorkModule::on_flag(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_HIT_SHIELD_TO_END);
        }
    }
    else if step == *FIGHTER_FALCO_ILLUSION_STEP_END {
        if situation == *SITUATION_KIND_AIR {
...
pub unsafe extern "C" fn special_s_handle_step(fighter: &mut L2CFighterCommon) {
    let step = WorkModule::get_int(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
    let situation = fighter.global_table[SITUATION_KIND].get_i32();
    WorkModule::set_int(fighter.module_accessor, step, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP_PREV);
    if step == *FIGHTER_FALCO_ILLUSION_STEP_START {
        let illusion_stop_y_frame = WorkModule::get_param_int(fighter.module_accessor, hash40("param_special_s"), hash40("illusion_stop_y_frame"));
        WorkModule::set_int(fighter.module_accessor, illusion_stop_y_frame, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
        ...
        if situation != *SITUATION_KIND_AIR {
            ...
            let end_speed_param = if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_HIT_SHIELD) {
...
pub unsafe extern "C" fn special_s_air_control(fighter: &mut L2CFighterCommon) {
    if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_AIR_CONTROL) {
        sv_kinetic_energy!(
            reset_energy,
            fighter,
