...
unsafe extern "C" fn snake_catch_game(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 4.8, 0.0, 8.2, 0.0, Some(0.0), Some(8.2), Some(12.0), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
...
unsafe extern "C" fn snake_catch_dash_game(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 4.8, 0.0, 8.2, 7.0, Some(0.0), Some(8.2), Some(11.3), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    game_CaptureCutCommon(agent);
    wait(lua_state, 2.0);
    if is_excute(agent) {
...
unsafe extern "C" fn snake_catch_turn_game(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 8.0);
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 4.8, 0.0, 8.2, -11.0, Some(0.0), Some(8.2), Some(-17.1), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    game_CaptureCutCommon(agent);
    wait(lua_state, 1.0);
    if is_excute(agent) {
