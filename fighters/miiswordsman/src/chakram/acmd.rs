...
unsafe extern "C" fn game_fly(agent: &mut L2CAgentBase) {
  ...
  if !WorkModule::is_flag(boma, *WEAPON_MIISWORDSMAN_CHAKRAM_INSTANCE_WORK_ID_FLAG_FLICK){
      if is_excute(agent) {
          VarModule::on_flag(owner_module_accessor.object(), vars::miiswordsman::instance::SPECIAL_S3_CHAKRAM_STICK);
...
unsafe extern "C" fn game_hop(agent: &mut L2CAgentBase) {
  ...
  let owner_module_accessor = &mut *sv_battle_object::module_accessor((WorkModule::get_int(boma, *WEAPON_INSTANCE_WORK_ID_INT_LINK_OWNER)) as u32);
  if is_excute(agent) {
      VarModule::off_flag(owner_module_accessor.object(), vars::miiswordsman::instance::SPECIAL_S3_CHAKRAM_STICK);
  }
}
