...
#[acmd_script( agent = "marth", script = "sound_dash" , category = ACMD_SOUND , low_priority)]
unsafe fn dash_sound(fighter: &mut L2CAgentBase) {
    ...
    wait(lua_state, 9.0);
    if is_excute(fighter) {
        PLAY_STEP(fighter, Hash40::new("se_marth_step_left_m"));
    }
    wait(lua_state, 5.0);
    if is_excute(fighter) {
        PLAY_STEP(fighter, Hash40::new("se_marth_step_right_m"));
    }
}
...
#[acmd_script( agent = "marth", script = "game_escapeair" , category = ACMD_GAME , low_priority)]
