...
unsafe fn packun_spikeball_game_start(fighter: &mut L2CAgentBase) {
    ...
    let stance = if fighter.kind() == *FIGHTER_KIND_KIRBY {0} else {VarModule::get_int(owner_module_accessor.object(), vars::packun::instance::CURRENT_STANCE)};
    let scale = if fighter.kind() == *FIGHTER_KIND_KIRBY {1.0} else {VarModule::get_float(owner_module_accessor.object(), vars::packun::instance::PTOOIE_SCALE)};
    frame(lua_state, 1.0);
    if is_excute(fighter) {
