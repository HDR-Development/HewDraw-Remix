...
unsafe extern "C" fn special_lw_empty_main(agent: &mut L2CFighterCommon) -> L2CValue {
    ...
    let use_cancel_anim = ink_current >= ink_cost;
...
pub fn install() {
    smashline::Agent::new("inkling")
        ...
        .status(Init, *FIGHTER_INKLING_STATUS_KIND_SPECIAL_LW_THROW, special_lw_throw_init)
        .status(Main, *FIGHTER_INKLING_STATUS_KIND_SPECIAL_LW_EMPTY, special_lw_empty_main)
    .install();
}