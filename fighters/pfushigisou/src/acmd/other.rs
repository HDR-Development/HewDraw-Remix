...
#[acmd_script( agent = "pfushigisou", script = "sound_damageflyhi" , category = ACMD_SOUND , low_priority)]
unsafe fn damageflyhi_sound(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        if !StopModule::is_stop(fighter.module_accessor) {
            ...
            if play_vc == 0 {PLAY_FLY_VOICE(fighter, Hash40::new("seq_pfushigisou_rnd_futtobi01"), Hash40::new("seq_pfushigisou_rnd_futtobi02"));}
        }
    }
    frame(lua_state, 1.1);
    if is_excute(fighter) {
...
#[acmd_script( agent = "pfushigisou", script = "sound_damageflylw" , category = ACMD_SOUND , low_priority)]
...
#[acmd_script( agent = "pfushigisou", script = "sound_damageflyn" , category = ACMD_SOUND , low_priority)]
...
#[acmd_script( agent = "pfushigisou", script = "sound_damageflyroll" , category = ACMD_SOUND , low_priority)]
...
#[acmd_script( agent = "pfushigisou", script = "sound_damageflytop" , category = ACMD_SOUND , low_priority)]
...
#[acmd_script( agent = "pfushigisou", script = "game_catch" , category = ACMD_GAME , low_priority)]
unsafe fn pfushigisou_catch_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    frame(lua_state, 1.0);
    if is_excute(fighter) {
    ...
    if is_excute(fighter) {
        ...
        CATCH(fighter, 0, Hash40::new("top"), 4.3, 0.0, 5.0, 0.0, Some(0.0), Some(5.0), Some(20.200001), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
...
#[acmd_script( agent = "pfushigisou", script = "game_escapeair" , category = ACMD_GAME , low_priority)]
