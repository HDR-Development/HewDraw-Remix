...
unsafe extern "C" fn special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    if MotionModule::is_end(fighter.module_accessor) {
        fighter.change_status(FIGHTER_PFUSHIGISOU_STATUS_KIND_SPECIAL_N_END.into(), false.into());
        return 0.into();
    }
...
pub fn install(agent: &mut Agent) {
    ...
    agent.status(Main, *FIGHTER_PFUSHIGISOU_STATUS_KIND_SPECIAL_N_END, special_n_end_main);
}