...
unsafe extern "C" fn daisy_special_lw_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn daisy_special_lw_main(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn daisy_special_lw_end(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    smashline::Agent::new("daisy")
        .status(Pre, *FIGHTER_STATUS_KIND_SPECIAL_LW, daisy_special_lw_pre)
        .status(Main, *FIGHTER_STATUS_KIND_SPECIAL_LW, daisy_special_lw_main)
        .status(End, *FIGHTER_STATUS_KIND_SPECIAL_LW, daisy_special_lw_end)
        .install();
}
