...
unsafe extern "C" fn special_s_hit_end_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if prev_inflict_status & *COLLISION_KIND_MASK_SHIELD | *COLLISION_KIND_MASK_PARRY != 0 {
        ...
        let accel_mul = ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.shield_hit_end_accel_x_mul");
...
pub fn install(agent: &mut Agent) {
    ...
    agent.status(Main, *FIGHTER_PEACH_STATUS_KIND_SPECIAL_S_HIT_END, special_s_hit_end_main);
}
