...
unsafe extern "C" fn daisy_catch_game(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        CATCH(fighter, 0, Hash40::new("top"), 4.0, 0.0, 8.5, 0.0, Some(0.0), Some(8.5), Some(7.5), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
...
unsafe extern "C" fn daisy_catch_dash_game(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn daisy_catch_turn_game(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.boma();
    frame(lua_state, 9.0);
    if is_excute(agent) {
...
pub fn install() {
    smashline::Agent::new("daisy")
        ...
        .acmd("game_catchdash", daisy_catch_dash_game)
        .acmd("game_catchturn", daisy_catch_turn_game)
