        ...
        if !fighter.is_pad_flag(PadFlag::AttackTrigger) {
            if *PACMAN_SPECIAL_N_RANK_MAX <= charge_rank {
                ...
                if max_hold_count == 0 {
                    ...
                    fighter.push_lua_stack(&mut L2CValue::I32(*EFFECT_SUB_ATTRIBUTE_NONE));
                    fighter.push_lua_stack(&mut L2CValue::I32(0));
                    fighter.push_lua_stack(&mut L2CValue::I32(0));
                    app::sv_module_access::effect(fighter.lua_state_agent);
