...
unsafe extern "C" fn lloid_callback(weapon : &mut L2CFighterBase) {
    ...
    if [*WEAPON_SHIZUE_CLAYROCKET_STATUS_KIND_BURST,
        *WEAPON_SHIZUE_CLAYROCKET_STATUS_KIND_DISAPPEAR].contains(&status) {
        VarModule::off_flag(shizue, vars::shizue::instance::SPECIAL_LW_LLOID_ASYNC);
        VarModule::set_int(shizue, vars::shizue::instance::SPECIAL_LW_LLOID_TIMER, 0);
    }
    if status == *WEAPON_SHIZUE_CLAYROCKET_STATUS_KIND_READY {
        if shizue_boma.is_cat_flag(Cat1::SpecialLw)
        && !VarModule::is_flag(shizue, vars::shizue::instance::SPECIAL_LW_LLOID_ASYNC)
    ...
    if VarModule::is_flag(shizue, vars::shizue::instance::SPECIAL_LW_LLOID_ASYNC) {
        if VarModule::get_int(shizue, vars::shizue::instance::SPECIAL_LW_LLOID_TIMER) > 0 {
            VarModule::dec_int(shizue, vars::shizue::instance::SPECIAL_LW_LLOID_TIMER);
        }
        else {
            VarModule::off_flag(shizue, vars::shizue::instance::SPECIAL_LW_LLOID_ASYNC);
