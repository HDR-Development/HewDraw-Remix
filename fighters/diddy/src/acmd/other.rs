...
unsafe extern "C" fn sound_dash(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(agent) {
        let dash_sfx_handle = SoundModule::play_se(boma, Hash40::new("se_diddy_dash_start"), true, false, false, false, app::enSEType(0));
...
pub fn install(agent: &mut Agent) {
    ...
    agent.acmd("game_appealhir", game_appealhil, Priority::Low);
}
