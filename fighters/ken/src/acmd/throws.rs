...
unsafe extern "C" fn game_throwhi(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        MeterModule::watch_damage(fighter.battle_object, true);
        ATTACK_ABS(fighter, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 8.0, 86, 75, 0, 55, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, attr, *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
