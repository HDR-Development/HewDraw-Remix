...
unsafe extern "C" fn trail_catch_dash_game(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn trail_catch_turn_game(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.boma();
    frame(lua_state, 1.0);
    FT_MOTION_RATE(agent, 0.5);
...
pub fn install() {
    smashline::Agent::new("trail")
        ...
        .acmd("game_catchdash", trail_catch_dash_game)
        .acmd("game_catchturn", trail_catch_turn_game)
