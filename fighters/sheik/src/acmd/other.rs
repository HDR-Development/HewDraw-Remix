...
unsafe extern "C" fn sheik_landing_air_lw_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
...
unsafe extern "C" fn sheik_catch_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    frame(lua_state, 6.0);
    if is_excute(fighter) {
...
pub fn install() {
    smashline::Agent::new("sheik_needle")
        .acmd("game_move", sheik_needle_move_game)
        .install();
    smashline::Agent::new("sheik")
        ...
        .acmd("game_landingairlw", sheik_landing_air_lw_game)
        .acmd("game_catch", sheik_catch_game)
