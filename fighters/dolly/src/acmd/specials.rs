...
unsafe extern "C" fn game_specialsfstart(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 1.0);
    if is_excute(agent) {
        VarModule::off_flag(agent.battle_object, vars::shotos::instance::EX_SPECIAL_USED);
    ...
    if is_excute(agent) {
        ...
        if (ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL) || ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL_RAW)) && ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_ATTACK) {
            if MeterModule::drain(agent.battle_object, 1) {
                ...
                VarModule::on_flag(agent.battle_object, vars::shotos::instance::EX_SPECIAL_USED);
            }
        }
    }
    frame(lua_state, 7.0);
...
unsafe extern "C" fn game_speciallwstart(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if (ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL) || ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL_RAW)) && ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_ATTACK) {
            ...
            if MeterModule::drain(agent.battle_object, 1) {
                VarModule::on_flag(agent.battle_object, vars::shotos::instance::ENABLE_TARGET_COMBO_1);
            }
        }
    }
}
...
unsafe extern "C" fn game_specialairlwrisew(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        if WorkModule::is_flag(boma, *FIGHTER_DOLLY_STATUS_SPECIAL_COMMON_WORK_FLAG_COMMAND) || VarModule::is_flag(agent.battle_object, vars::shotos::instance::EX_SPECIAL_USED) {
...
unsafe extern "C" fn effect_specialairlwrise(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        else if VarModule::is_flag(agent.battle_object, vars::shotos::instance::ENABLE_TARGET_COMBO_1) {
            EFFECT_FOLLOW(agent, Hash40::new("sys_sp_flash"), Hash40::new("top"), 3.0, 10.0, 0, 0, 0, 0, 1.0, true);
            LAST_EFFECT_SET_RATE(agent, 0.8);
            EFFECT_FOLLOW(agent, Hash40::new("sys_damage_aura"), Hash40::new("kneer"), 3.0, 10.0, 0, 0, 0, 0, 1.0, true);
            LAST_EFFECT_SET_RATE(agent, 1.1);
...
unsafe extern "C" fn game_speciallwend(agent: &mut L2CAgentBase) {
    ...
    let boma = agent.boma();
    if is_excute(agent) {
        VarModule::off_flag(agent.battle_object, vars::shotos::instance::ENABLE_TARGET_COMBO_2);
        if VarModule::is_flag(agent.battle_object, vars::shotos::instance::EX_SPECIAL_USED) {
            AttackModule::clear_all(boma);
            ...
            if (ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL) || ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL_RAW)) && ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_ATTACK) {
                if MeterModule::drain(agent.battle_object, 3) {
                    VarModule::on_flag(boma.object(), vars::shotos::instance::ENABLE_TARGET_COMBO_2);
                }
            }
            if VarModule::is_flag(agent.battle_object, vars::shotos::instance::ENABLE_TARGET_COMBO_2) {
...
pub fn install(agent: &mut Agent) {
    ...
    agent.acmd("effect_specialsfstart", effect_specialsfstart, Priority::Low);
    agent.acmd("game_specialairsfstart", game_specialairsfstart, Priority::Low);
    agent.acmd("effect_specialairsfstart", effect_specialairsfstart, Priority::Low);

    agent.acmd("game_specialsfattack", game_specialsfattack, Priority::Low);
    agent.acmd("effect_specialsfattack", effect_specialsfattack, Priority::Low);
    ...
    agent.acmd("effect_specialsbstart", effect_specialsbstart, Priority::Low);
    agent.acmd("game_specialairsbstart", game_specialairsbstart, Priority::Low);
    agent.acmd("effect_specialairsbstart", effect_specialairsbstart, Priority::Low);

    agent.acmd("game_specialsbattackw", game_specialsbattackw, Priority::Low);
    agent.acmd("effect_specialsbattackw", effect_specialsbattackw, Priority::Low);
    ...
    agent.acmd("effect_specialairlwrisew", effect_specialairlwrisew, Priority::Low);

    agent.acmd("game_specialairlwrise", game_specialairlwrise, Priority::Low);
    agent.acmd("effect_specialairlwrise", effect_specialairlwrise, Priority::Low);
