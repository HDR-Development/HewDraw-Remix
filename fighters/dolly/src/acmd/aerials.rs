...
unsafe extern "C" fn game_attackairf(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        WorkModule::on_flag(boma, *FIGHTER_DOLLY_INSTANCE_WORK_ID_FLAG_FINAL_HIT_CANCEL);
        WorkModule::on_flag(boma, *FIGHTER_DOLLY_STATUS_ATTACK_WORK_FLAG_HIT_CANCEL);
        VarModule::off_flag(boma.object(), vars::shotos::status::SCALE_COMBO_DAMAGE);
        if VarModule::is_flag(boma.object(), vars::shotos::instance::MAGIC_SERIES_CANCEL){
            VarModule::off_flag(boma.object(), vars::shotos::instance::MAGIC_SERIES_CANCEL);
            VarModule::on_flag(boma.object(), vars::shotos::status::SCALE_COMBO_DAMAGE);
        }
    }
    frame(lua_state, 1.0);
    if is_excute(agent) {
    ...
    frame(lua_state, 5.0);
    if is_excute(agent) {
        if VarModule::is_flag(boma.object(), vars::shotos::status::SCALE_COMBO_DAMAGE){
