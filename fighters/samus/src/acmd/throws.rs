...
unsafe fn game_throwhi(fighter: &mut L2CAgentBase) {
    if is_excute(fighter) {
        ATTACK_ABS(fighter, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 4.0, 92, 75, 0, 100, 0.0, 0.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
        ATTACK_ABS(fighter, *FIGHTER_ATTACK_ABSOLUTE_KIND_CATCH, 0, 3.0, 361, 75, 0, 100, 0.0, 0.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    ...
    if is_excute(fighter) {
        ...
        if ArticleModule::is_exist(fighter.module_accessor, weapon) {
...
unsafe fn effect_throwhi(fighter: &mut L2CAgentBase) {
    ...
    frame(fighter.lua_state_agent , 15.0);
    if is_excute(fighter) {
        EFFECT(fighter, Hash40::new("samus_missile_shot"), Hash40::new("haver"), 2.2, 0.379, -0.15, 0, 0, 90, 1, 0, 0, 0, 0, 0, 0, false);
    }
    wait(fighter.lua_state_agent , 1.0);
    if is_excute(fighter) {
        EFFECT(fighter, Hash40::new("samus_missile_straight_smoke"), Hash40::new("armr"), 0, 0, 4, 0, 0, 90, 0.8, 0, 0, 0, 0, 0, 0, false);
    }
