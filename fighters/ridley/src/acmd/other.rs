...
unsafe extern "C" fn ridley_catch_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    frame(lua_state, 1.0);
    if is_excute(fighter) {
...
unsafe extern "C" fn ridley_breath_fly_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    if is_excute(fighter) {
...
pub fn install() {
    smashline::Agent::new("ridley")
        ...
        .acmd("game_catch", ridley_catch_game)
        ...
        .install();
    smashline::Agent::new("ridley_breath")
        .acmd("game_fly", ridley_breath_fly_game)
        .install();
}
