...
unsafe extern "C" fn elight_catch_game(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn elight_catch_dash_game(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.boma();
    frame(lua_state, 1.0);
...
unsafe extern "C" fn elight_catch_turn_game(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.boma();
    frame(lua_state, 1.0);
    FT_MOTION_RATE(agent, 0.5);
...
pub fn install() {
    smashline::Agent::new("elight")
        .acmd("game_catch", elight_catch_game)
        .acmd("game_catchdash", elight_catch_dash_game)
        .acmd("game_catchturn", elight_catch_turn_game)
