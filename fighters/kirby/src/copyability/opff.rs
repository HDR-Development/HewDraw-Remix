...
//Fastfall Specials
unsafe fn fastfall_copyspecials(fighter: &mut L2CFighterCommon) {
    let copystatus = StatusModule::status_kind(fighter.module_accessor);
    if !fighter.is_in_hitlag()
    && !StatusModule::is_changing(fighter.module_accessor)
    && fighter.is_situation(*SITUATION_KIND_AIR) {
        if fighter.is_status_one_of(&[
            // for some reason these two statuses aren't considered even if they are in the range, so the range does not include these two
            *FIGHTER_KIRBY_STATUS_KIND_TRAIL_SPECIAL_N2,
            *FIGHTER_KIRBY_STATUS_KIND_TRAIL_SPECIAL_N3
            ])
        || ((0x206..0x377).contains(&copystatus)
        && !fighter.is_status_one_of(&[
            // to not list hundreds of statuses, range is given, but exceptions need to be listed seperately
...
unsafe fn koopa_fireball_cooldown(boma: &mut BattleObjectModuleAccessor, status_kind: i32) {
    ...
    // Kirby specific, resets gimmick if copy ability isn't enabled
    if WorkModule::get_int(boma, *FIGHTER_KIRBY_INSTANCE_WORK_ID_INT_COPY_CHARA) != FIGHTER_KIND_KOOPA {
        VarModule::set_int(boma.object(), vars::koopa::instance::FIREBALL_COOLDOWN_FRAME,KOOPA_MAX_COOLDOWN);
    }
}
...
pub unsafe fn lucas_offense_charge(fighter: &mut smash::lua2cpp::L2CFighterCommon, boma: &mut BattleObjectModuleAccessor, situation_kind: i32, status_kind: i32)  {
...
pub unsafe fn copymoveset(fighter: &mut L2CFighterCommon, boma: &mut BattleObjectModuleAccessor, id: usize, cat: [i32 ; 4], status_kind: i32, situation_kind: i32, motion_kind: u64, stick_x: f32, stick_y: f32, facing: f32, frame: f32) {
    // Common
...
pub unsafe fn kirby_frame(fighter: &mut smash::lua2cpp::L2CFighterCommon) {
    if let Some(info) = FrameInfo::update_and_get(fighter) {
        copymoveset(fighter, &mut *info.boma, info.id, info.cat, info.status_kind, info.situation_kind, info.motion_kind.hash, info.stick_x, info.stick_y, info.facing, info.frame);
    }
}
