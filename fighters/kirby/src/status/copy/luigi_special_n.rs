...
unsafe extern "C" fn special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.status_frame() == 10 && (ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_SPECIAL) || ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_SPECIAL_RAW)) {
        VarModule::on_flag(fighter.object(), vars::kirby::status::SPECIAL_N_THUNDERHAND);
    ...
    if !StatusModule::is_changing(fighter.module_accessor) {
        if StatusModule::is_situation_changed(fighter.module_accessor) {
            if fighter.is_situation(*SITUATION_KIND_GROUND) {
                ...
                KineticModule::change_kinetic(fighter.module_accessor, *FIGHTER_KINETIC_TYPE_GROUND_STOP);
                let motion = if VarModule::is_flag(fighter.object(), vars::kirby::status::SPECIAL_N_THUNDERHAND) { Hash40::new("luigi_special_n_thunder") } else { Hash40::new("luigi_special_n") };
                MotionModule::change_motion_inherit_frame(fighter.module_accessor, motion, -1.0, 1.0, 0.0, false, false);
            }
            else {
                GroundModule::correct(fighter.module_accessor, smash::app::GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
                KineticModule::change_kinetic(fighter.module_accessor, *FIGHTER_KINETIC_TYPE_AIR_STOP);
                let motion = if VarModule::is_flag(fighter.object(), vars::kirby::status::SPECIAL_N_THUNDERHAND) { Hash40::new("luigi_special_air_n_thunder") } else { Hash40::new("luigi_special_air_n") };
                MotionModule::change_motion_inherit_frame(fighter.module_accessor, motion, -1.0, 1.0, 0.0, false, false);
            }
        }
    }
