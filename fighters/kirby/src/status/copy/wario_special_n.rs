...
unsafe extern "C" fn special_n_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    let ret = smashline::original_status(Main, fighter, *FIGHTER_KIRBY_STATUS_KIND_WARIO_SPECIAL_N)(fighter);
...
unsafe extern "C" fn special_n_open_wait_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    let ret = smashline::original_status(Main, fighter, *FIGHTER_KIRBY_STATUS_KIND_WARIO_SPECIAL_N_OPEN_WAIT)(fighter);
    special_n_drift(fighter, 0.8, 0.5);
    ret
}

unsafe extern "C" fn special_n_bite_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    let ret = smashline::original_status(Main, fighter, *FIGHTER_KIRBY_STATUS_KIND_WARIO_SPECIAL_N_BITE)(fighter);
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_KIRBY_STATUS_KIND_WARIO_SPECIAL_N, special_n_main);
    agent.status(Main, *FIGHTER_KIRBY_STATUS_KIND_WARIO_SPECIAL_N_OPEN_WAIT, special_n_open_wait_main);
    agent.status(Main, *FIGHTER_KIRBY_STATUS_KIND_WARIO_SPECIAL_N_BITE, special_n_bite_main);
}