...
unsafe extern "C" fn pzenigame_special_n_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn pzenigame_special_n_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    special_n_motion_helper(fighter, false, Hash40::new("special_n_start"), Hash40::new("special_air_n_start"));
    fighter.main_shift(pzenigame_special_n_main_loop)
}

unsafe extern "C" fn pzenigame_special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.sub_air_check_dive();
    if MotionModule::is_end(fighter.module_accessor) {
        fighter.change_status(FIGHTER_KIRBY_STATUS_KIND_PZENIGAME_SPECIAL_N_SHOOT.into(), false.into());
        return 0.into();
    }
...
unsafe extern "C" fn pzenigame_special_n_shoot_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    special_n_motion_helper(fighter, false, Hash40::new("special_n_shot"), Hash40::new("special_air_n_shot"));
    fighter.main_shift(pzenigame_special_n_shoot_main_loop)
}

unsafe extern "C" fn pzenigame_special_n_shoot_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.sub_air_check_dive();
...
// unsafe extern "C" fn pzenigame_special_n_shoot_exec(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, *FIGHTER_KIRBY_STATUS_KIND_PZENIGAME_SPECIAL_N, pzenigame_special_n_pre);
    agent.status(Main, *FIGHTER_KIRBY_STATUS_KIND_PZENIGAME_SPECIAL_N, pzenigame_special_n_main);

    agent.status(Main, *FIGHTER_KIRBY_STATUS_KIND_PZENIGAME_SPECIAL_N_SHOOT, pzenigame_special_n_shoot_main);
    //agent.status(Exec, *FIGHTER_PZENIGAME_STATUS_KIND_SPECIAL_N_SHOOT, pzenigame_special_n_shoot_exec);
}