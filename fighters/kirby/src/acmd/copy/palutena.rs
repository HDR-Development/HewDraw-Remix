...
unsafe extern "C" fn effect_palutenaspecialn(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        if WorkModule::is_flag(boma, *FIGHTER_PALUTENA_STATUS_SPECIAL_N_FLAG_HAS_TARGET_ARTICLE) {
            EFFECT_FOLLOW(agent, Hash40::new("palutena_wand_light_trace"), Hash40::new("havel"), 0, 6.5, 0, 0, 0, 0, 0.95, true);
            EffectModule::enable_sync_init_pos_last(boma);
            EFFECT_FOLLOW(agent, Hash40::new("palutena_lockon"), Hash40::new("havel"), 0, -2.5, 0, 0, 0, 0, 1, true);
        }
    }
