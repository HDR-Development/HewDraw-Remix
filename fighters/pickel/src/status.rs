...
pub unsafe fn jumpsquat(fighter: &mut L2CFighterCommon) -> L2CValue {
    if pickel_js_status_check(fighter).get_bool() {
        fighter.sub_shift_status_main(L2CValue::Ptr(smash::lua2cpp::L2CFighterCommon_status_JumpSquat_Main as *const () as _))
    }
    else {
...
pub unsafe fn exec_stop_jumpsquat(fighter: &mut L2CFighterCommon) -> L2CValue {
    if pickel_js_status_check(fighter).get_bool() {
        MotionModule::change_motion_inherit_frame(fighter.module_accessor, Hash40::new_raw(0xad160bda8), -1.0, 1.0, 0.0, true, true);
    }
    else {
...
unsafe extern "C" fn pickel_js_status_check(fighter: &mut L2CFighterCommon) -> L2CValue {
    if fighter.global_table[PREV_STATUS_KIND] != FIGHTER_PICKEL_STATUS_KIND_ATTACK_JUMP_SQUAT && fighter.global_table[PREV_STATUS_KIND] != FIGHTER_PICKEL_STATUS_KIND_SPECIAL_N1_JUMP_SQUAT && fighter.global_table[PREV_STATUS_KIND] != FIGHTER_PICKEL_STATUS_KIND_SPECIAL_N3_JUMP_SQUAT {
...
pub unsafe fn waza_jumpsquat(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let special_stick_x = WorkModule::get_param_float(fighter.module_accessor, hash40("common"), hash40("special_stick_x"));
...
pub unsafe fn jump(fighter: &mut L2CFighterCommon) -> L2CValue {
    if pickel_jump_status_check(fighter).get_bool() {
            ...
            if MotionModule::motion_kind(fighter.module_accessor) != 0x17f0bb63e4u64 {
                if MotionModule::motion_kind(fighter.module_accessor) !=0x12e6fa5eceu64 {
                    if MotionModule::motion_kind(fighter.module_accessor) != 0x176b2a21f2u64 {
                        MotionModule::change_motion_inherit_frame(fighter.module_accessor, Hash40::new_raw(0x62dd02058), -1.0, 1.0, 0.0, true, true);
                    }
                    else {
                        MotionModule::change_motion_inherit_frame(fighter.module_accessor, Hash40::new_raw(0xb38c9ab48), -1.0, 1.0, 0.0, true, true);
                    }
                }
                else {
                    MotionModule::change_motion_inherit_frame(fighter.module_accessor, Hash40::new_raw(0x62abde441), -1.0, 1.0, 0.0, true, true);
                }
            }
            else {
                MotionModule::change_motion_inherit_frame(fighter.module_accessor, Hash40::new_raw(0xba358e95e), -1.0, 1.0, 0.0, true, true);
            }
            if !StopModule::is_stop(fighter.module_accessor) {
                fighter.sub_fall_common_uniq(L2CValue::Bool(false));
            }
            fighter.global_table[SUB_STATUS].assign(&L2CValue::Ptr(smash::lua2cpp::L2CFighterCommon_sub_fall_common_uniq as *const () as _));
            fighter.sub_shift_status_main(L2CValue::Ptr(smash::lua2cpp::L2CFighterCommon_status_Jump_Main as *const () as _))
        }
        else {
            return 0.into()
        }
        ...
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_STATUS_WORK_ID_FLAG_RESERVE_JUMP_MINI) {
            let pickel_int = WorkModule::get_param_int(fighter.module_accessor, hash40("param_private"), 0xf9b69867e);
            WorkModule::set_int(fighter.module_accessor, pickel_int, *FIGHTER_PICKEL_INSTANCE_WORK_ID_INT_JUMP_MINI_FRAME);
        }
        fighter.status_Jump()
    }
}
...
unsafe extern "C" fn sub_jump(fighter: &mut L2CFighterCommon) -> L2CValue {
    let attack_cancel_status_kind = WorkModule::get_int(fighter.module_accessor, *FIGHTER_PICKEL_INSTANCE_WORK_ID_INT_ATTACK_CANCEL_STATUS_KIND);
    if attack_cancel_status_kind != *FIGHTER_STATUS_KIND_NONE {
        fighter.change_status(
            L2CValue::I32(attack_cancel_status_kind),
            L2CValue::Bool(true)
        );
        WorkModule::set_int(fighter.module_accessor, *FIGHTER_STATUS_KIND_NONE, *FIGHTER_PICKEL_INSTANCE_WORK_ID_INT_ATTACK_CANCEL_STATUS_KIND);
        return L2CValue::Bool(true);
    }
    else {
        return L2CValue::Bool(false);
...
unsafe extern "C" fn pickel_jump_status_check(fighter: &mut L2CFighterCommon) -> L2CValue {
    if fighter.global_table[PREV_STATUS_KIND] != FIGHTER_PICKEL_STATUS_KIND_ATTACK_JUMP && fighter.global_table[PREV_STATUS_KIND] != FIGHTER_PICKEL_STATUS_KIND_SPECIAL_N1_JUMP && fighter.global_table[PREV_STATUS_KIND] != FIGHTER_PICKEL_STATUS_KIND_SPECIAL_N3_JUMP {
        return L2CValue::Bool(false);
    }
    else {
        return L2CValue::Bool(true);
