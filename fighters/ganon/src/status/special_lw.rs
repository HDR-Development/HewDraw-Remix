...
unsafe extern "C" fn special_lw_end_main(agent: &mut L2CFighterCommon) -> L2CValue {
    let x_speed = KineticModule::get_sum_speed_x(agent.module_accessor, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);

    let ret = smashline::original_status(Main, agent, *FIGHTER_GANON_STATUS_KIND_SPECIAL_LW_END)(agent);

    let start_situation = WorkModule::get_int(agent.module_accessor, *FIGHTER_GANON_STATUS_WORK_ID_INT_GANON_KICK_START_SITUATION);
    let end_situation = WorkModule::get_int(agent.module_accessor, *FIGHTER_GANON_STATUS_WORK_ID_INT_GANON_KICK_END_SITUATION);
    ...
    && end_situation == *FIGHTER_GANON_KICK_END_SITUATION_AG {
        ...
        KineticModule::add_speed(agent.module_accessor, &Vector3f{x: x_speed.abs(), y: 0.0, z: 0.0});
    }
    ...
    && end_situation == *FIGHTER_GANON_KICK_END_SITUATION_AA {
        ...
        WorkModule::set_int(agent.module_accessor, 0, *FIGHTER_GANON_STATUS_WORK_ID_INT_GANON_KICK_START_SITUATION);
    }
