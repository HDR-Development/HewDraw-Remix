...
#[acmd_script( agent = "pikachu", script = "sound_damageflyhi" , category = ACMD_SOUND , low_priority)]
unsafe fn damageflyhi_sound(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        if !StopModule::is_stop(fighter.module_accessor) {
            ...
            if play_vc == 0 {PLAY_FLY_VOICE(fighter, Hash40::new("seq_pikachu_rnd_futtobi01"), Hash40::new("seq_pikachu_rnd_futtobi02"));}
        }
    }
    frame(lua_state, 1.1);
    if is_excute(fighter) {
...
#[acmd_script( agent = "pikachu", script = "sound_damageflylw" , category = ACMD_SOUND , low_priority)]
...
#[acmd_script( agent = "pikachu", script = "sound_damageflyn" , category = ACMD_SOUND , low_priority)]
...
#[acmd_script( agent = "pikachu", script = "sound_damageflytop" , category = ACMD_SOUND , low_priority)]
...
unsafe fn dash_sound(fighter: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(fighter) {
        let dash_sfx_handle = SoundModule::play_se(fighter.module_accessor, Hash40::new("se_pikachu_dash_start"), true, false, false, false, app::enSEType(0));
...
unsafe fn pikachu_kaminari_regular_game(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        ...
        AttackModule::set_vec_target_pos(boma, 0, Hash40::new("top"), &Vector2f::new(0.0, -100.0), 24, false);
    }
    frame(lua_state, 4.0);
    if is_excute(fighter) {
        ATTACK(fighter, 0, 0, Hash40::new("top"), 8.0, 70, 60, 0, 74, 7.0, 0.0, 4.0, 0.0, Some(0.0), Some(14.0), Some(0.0), 1.0, 1.0, *ATTACK_SETOFF_KIND_OFF, *ATTACK_LR_CHECK_POS, false, 1, 0.0, 0, true, true, false, false, false, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_elec"), *ATTACK_SOUND_LEVEL_L, *COLLISION_SOUND_ATTR_ELEC, *ATTACK_REGION_NONE);
    }        

}

#[acmd_script( agent = "pikachu", script = "game_escapeair" , category = ACMD_GAME , low_priority)]
