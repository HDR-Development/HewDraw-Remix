...
unsafe extern "C" fn special_n_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if curr_charge_frame >= max_charge_frame {
        fighter.on_flag(*FIGHTER_MEWTWO_SPECIAL_N_STATUS_WORK_ID_FLAG_CHARGE_MAX);
    } else {
        WorkModule::enable_transition_term(fighter.module_accessor, *FIGHTER_MEWTWO_SHADOWBALL_TRANSITION_TERM_ID_START_HOLD);
        WorkModule::unable_transition_term(fighter.module_accessor, *FIGHTER_MEWTWO_SHADOWBALL_TRANSITION_TERM_ID_START_SHOOT);
    }
...
unsafe extern "C" fn special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    if !StatusModule::is_changing(fighter.module_accessor) {
        ...
        && fighter.is_button_trigger(Buttons::Special) {
            WorkModule::enable_transition_term(fighter.module_accessor, *FIGHTER_MEWTWO_SHADOWBALL_TRANSITION_TERM_ID_START_SHOOT);
            WorkModule::unable_transition_term(fighter.module_accessor, *FIGHTER_MEWTWO_SHADOWBALL_TRANSITION_TERM_ID_START_HOLD);
        }
    ...
    if MotionModule::is_end(fighter.module_accessor) {
        ...
        if fighter.is_flag(*FIGHTER_MEWTWO_SPECIAL_N_STATUS_WORK_ID_FLAG_CHARGE_MAX) {
            ...
            ArticleModule::change_status(
                ...
                *WEAPON_MEWTWO_SHADOWBALL_STATUS_KIND_CHARGE,
...
unsafe extern "C" fn special_n_check_cancel(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.is_situation(*SITUATION_KIND_GROUND) {
        ...
        //         fighter.set_int(*FIGHTER_STATUS_KIND_ESCAPE, *FIGHTER_MEWTWO_SPECIAL_N_STATUS_WORK_ID_INT_CANCEL_STATUS);
        //     } else {
        //         fighter.set_int(*STATUS_KIND_NONE, *FIGHTER_MEWTWO_SPECIAL_N_STATUS_WORK_ID_INT_CANCEL_STATUS);
        //     }
        //     return true.into();
        ...
        //         fighter.set_int(*FIGHTER_STATUS_KIND_ESCAPE_F, *FIGHTER_MEWTWO_SPECIAL_N_STATUS_WORK_ID_INT_CANCEL_STATUS);
        //     } else {
        //         fighter.set_int(*STATUS_KIND_NONE, *FIGHTER_MEWTWO_SPECIAL_N_STATUS_WORK_ID_INT_CANCEL_STATUS);
        //     }
        //     return true.into();
        ...
        //         fighter.set_int(*FIGHTER_STATUS_KIND_ESCAPE_B, *FIGHTER_MEWTWO_SPECIAL_N_STATUS_WORK_ID_INT_CANCEL_STATUS);
        //     } else {
        //         fighter.set_int(*STATUS_KIND_NONE, *FIGHTER_MEWTWO_SPECIAL_N_STATUS_WORK_ID_INT_CANCEL_STATUS);
        //     }
        //     return true.into();
...
unsafe extern "C" fn special_n_set_kinetic(fighter: &mut L2CFighterCommon) {
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        ...
        let mot = WorkModule::get_int64(fighter.module_accessor, *FIGHTER_MEWTWO_INSTANCE_WORK_ID_INT_AIR_MOT);
        ...
        let mot = WorkModule::get_int64(fighter.module_accessor, *FIGHTER_MEWTWO_INSTANCE_WORK_ID_INT_GROUND_MOT);
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_MEWTWO_INSTANCE_WORK_ID_FLAG_MOT_INHERIT) {
            MotionModule::change_motion_inherit_frame_keep_rate(
                fighter.module_accessor,
                Hash40::new_raw(mot),
...
pub unsafe extern "C" fn special_n_save_charge_status(fighter: &mut L2CFighterCommon) {
    ...
    || [special_n_hold, special_n_max, special_n_shoot, special_n_cancel].contains(&next_status) {
        let article = ArticleModule::get_article(fighter.module_accessor, *FIGHTER_MEWTWO_GENERATE_ARTICLE_SHADOWBALL);
        if !article.is_null() {
            ...
            let charge_frame = WorkModule::get_int(article_module_accessor, *WEAPON_MEWTWO_SHADOWBALL_INSTANCE_WORK_ID_INT_CHARGE_FRAME);
    ...
    let is_exist = ArticleModule::is_exist(fighter.module_accessor, *FIGHTER_MEWTWO_GENERATE_ARTICLE_SHADOWBALL);
    ...
    if is_exist && ![special_n_hold, special_n_max, special_n_shoot].contains(&next_status) {
        ArticleModule::remove_exist(fighter.module_accessor, *FIGHTER_MEWTWO_GENERATE_ARTICLE_SHADOWBALL, ArticleOperationTarget(0));
    }
