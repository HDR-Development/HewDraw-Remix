...
unsafe fn pichu_dengekidama_game_regular(fighter: &mut L2CAgentBase) {
    ...
    if !VarModule::is_flag(owner_module_accessor.object(), vars::pichu::status::IS_CHARGE_ATTACK) {
        if is_excute(fighter) {
    ...
    else if VarModule::is_flag(owner_module_accessor.object(), vars::pichu::status::IS_CHARGE_ATTACK) {
        if is_excute(fighter) {
