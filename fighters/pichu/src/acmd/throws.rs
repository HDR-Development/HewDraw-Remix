...
unsafe fn pichu_throw_b_game(fighter: &mut L2CAgentBase) {
    ...
    if charged {
        FT_MOTION_RATE(fighter, 18.0/(26.0-12.0));
        if is_excute(fighter){
...
unsafe fn pichu_throw_hi_game(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        if charged {
            ATTACK_ABS(fighter, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 6.0, 90, 45, 0, 70, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_elec"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_ELEC, *ATTACK_REGION_THROW);
        }
        else {
...
unsafe fn game_throwlw(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        if charged{
            ATTACK_ABS(fighter, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 12.0, 60, 50, 0, 60, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
        }
        else{ 
    ...
    frame(lua_state, 7.0);
    if charged {
        FT_MOTION_RATE(fighter, (20.0/(14.0-7.0)));
    }
    frame(lua_state, 14.0);
