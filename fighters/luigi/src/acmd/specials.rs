...
unsafe extern "C" fn game_specialairsstart(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 3.0);
    if is_excute(agent) {
        WorkModule::on_flag(agent.module_accessor, *FIGHTER_LUIGI_STATUS_SPECIAL_S_FLAG_REVERSE_LR);
    }
}
...
unsafe extern "C" fn effect_specialshold(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        let handle = EffectModule::get_last_handle(agent.module_accessor) as u32;
        VarModule::set_int(agent.battle_object, vars::luigi::instance::CHARGE_PULSE_EFFECT_HANDLE, handle as i32);
        if WorkModule::is_flag(agent.module_accessor, *FIGHTER_LUIGI_STATUS_SPECIAL_S_CHARGE_FLAG_DISCHARGE) {
