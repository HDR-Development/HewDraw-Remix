...
unsafe extern "C" fn sound_damagefly(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if VarModule::is_flag(agent.battle_object, vars::common::instance::IS_KILLING_BLOW) {
            PLAY_SE(agent, Hash40::new("vc_popo_damagefly02"));
        } else {
...
unsafe extern "C" fn sound_damagefly_nana(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if VarModule::is_flag(agent.battle_object, vars::common::instance::IS_KILLING_BLOW) {
            PLAY_SE(agent, Hash40::new("vc_nana_damagefly02"));
        } else {
            let damage_speed_x = agent.get_speed_x(*FIGHTER_KINETIC_ENERGY_ID_DAMAGE);
            let damage_speed_y = agent.get_speed_y(*FIGHTER_KINETIC_ENERGY_ID_DAMAGE);
...
unsafe extern "C" fn sound_dash(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(agent) {
        let dash_sfx_handle = SoundModule::play_se(boma, Hash40::new("se_popo_dash_start"), true, false, false, false, app::enSEType(0));
...
pub fn install(agent: &mut Agent) {
    ...
    agent.acmd("sound_damageflyhi_nana", sound_damagefly_nana, Priority::Low);
    agent.acmd("sound_damageflylw_nana", sound_damagefly_nana, Priority::Low);
    agent.acmd("sound_damageflyn_nana", sound_damagefly_nana, Priority::Low);
    agent.acmd("sound_damageflyroll_nana", sound_damageflyroll_nana, Priority::Low);
    ...
    agent.acmd("game_cliffjump2_nana", game_cliffjump2, Priority::Low);
}
