...
unsafe extern "C" fn game_throwb(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        let dmg = if variation == 2 { 11.0 } else { 5.0 };
        ATTACK_ABS(agent, *WEAPON_PIKMIN_PIKMIN_ATTACK_ABSOLUTE_KIND_THROW, 0, dmg * p.dmg, 74 + p.angle, 49, 0, 84, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, p.attr, *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    }
...
unsafe extern "C" fn game_throwf(agent: &mut L2CAgentBase) {
    ...
    let p = PikminInfo::from(variation);
    if is_excute(agent) {
        let dmg = if variation == 2 { 13.0 } else { 6.0 };
        ATTACK_ABS(agent, *WEAPON_PIKMIN_PIKMIN_ATTACK_ABSOLUTE_KIND_THROW, 0, dmg * p.dmg, 45, 68, 0, 75, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, p.attr, *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    }
...
unsafe extern "C" fn game_throwhi(agent: &mut L2CAgentBase) {
    ...
    let p = PikminInfo::from(variation);
    if is_excute(agent) {
        let dmg = if variation == 2 { 12.0 } else { 7.0 };
        ATTACK_ABS(agent, *WEAPON_PIKMIN_PIKMIN_ATTACK_ABSOLUTE_KIND_THROW, 0, dmg * p.dmg, 82 + p.angle, 65, 0, 90, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, p.attr, *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    }
    frame(lua_state, 20.0);
    if is_excute(agent) {
        let dmg = 1.0;
