...
unsafe extern "C" fn sound_damagefly(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if VarModule::is_flag(agent.battle_object, vars::common::instance::IS_KILLING_BLOW) {
            PLAY_SE(agent, Hash40::new("vc_lucas_damagefly02"));
        } else {
...
unsafe extern "C" fn sound_dash(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(agent) {
        let dash_sfx_handle = SoundModule::play_se(boma, Hash40::new("se_lucas_dash_start"), true, false, false, false, app::enSEType(0));
...
unsafe extern "C" fn game_appealhir(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if agent.is_button_on(Buttons::AppealHi) {
            StatusModule::change_status_request_from_script(boma, *FIGHTER_STATUS_KIND_SLIP_WAIT, true);
        }
    }
}
...
pub fn install(agent: &mut Agent) {
    ...
    agent.acmd("game_appeallwl", game_appeallwr, Priority::Low);
