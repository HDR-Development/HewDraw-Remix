...
unsafe extern "C" fn damageflyhi_sound(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        if !StopModule::is_stop(fighter.module_accessor) {
            ...
            if play_vc == 0 {PLAY_FLY_VOICE(fighter, Hash40::new("seq_gekkouga_rnd_futtobi01"), Hash40::new("seq_gekkouga_rnd_futtobi02"));}
        }
    }
    frame(lua_state, 1.1);
    if is_excute(fighter) {
...
unsafe extern "C" fn dash_sound(fighter: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(fighter) {
        let dash_sfx_handle = SoundModule::play_se(fighter.module_accessor, Hash40::new("se_gekkouga_dash_start"), true, false, false, false, app::enSEType(0));
