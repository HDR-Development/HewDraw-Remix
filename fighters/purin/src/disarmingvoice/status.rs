...
unsafe extern "C" fn shoot_init(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    let life = WorkModule::get_param_int(
        weapon.module_accessor,
        hash40("param_disarmingvoice"),
        hash40("life")
    );
    let speed_max = WorkModule::get_param_float(
        weapon.module_accessor,
        hash40("param_disarmingvoice"),
        hash40("speed_max")
    );
    ...
    PostureModule::set_pos(
        weapon.module_accessor,
        &(Vector3f { x: owner_pos_x, y: owner_pos_y + 7.0, z: owner_pos_z })
    );
    (0).into()
}
...
unsafe extern "C" fn shoot_main(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    if
        ...
        WorkModule::is_flag(
            weapon.module_accessor,
            *WEAPON_KOOPAJR_CANNONBALL_INSTANCE_WORK_ID_FLAG_HIT_WALL
        ) ||
        life <= 0
    {
        EffectModule::req(
    ...
    if
        AttackModule::is_infliction_status(
            weapon.module_accessor,
            statuses::purin_disarming_voice::SHOOT
        ) ||
        StopModule::is_stop(weapon.module_accessor) ||
        WorkModule::is_flag(
            weapon.module_accessor,
            *WEAPON_KOOPAJR_CANNONBALL_INSTANCE_WORK_ID_FLAG_ATTACK
        )
    {
        EffectModule::req(
            weapon.module_accessor,
...
unsafe extern "C" fn shoot_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    if
        ...
        (situation_kind == *SITUATION_KIND_GROUND &&
            ...
            VarModule::is_flag(purin, vars::purin::status::SPECIAL_N_AIR))
    {
        EffectModule::req(
            weapon.module_accessor,
            Hash40::new("sys_erace_smoke"),
