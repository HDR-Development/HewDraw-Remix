        ...
        if is_excute(agent) {
            if !boma.is_status(*WEAPON_DEDEDE_GORDO_STATUS_KIND_HOP) {
                if VarModule::is_flag(owner_module_accessor.object(), vars::dedede::instance::SPECIAL_S_GORDO_DASH_SUCCESS){
                    ...
                    if (WorkModule::get_int(boma, *WEAPON_DEDEDE_GORDO_STATUS_WORK_INT_BOUND_COUNT) - VarModule::get_int(owner_module_accessor.object(), vars::dedede::instance::SPECIAL_S_RECATCH_COUNT)) < 0{
