...
unsafe fn metaknight_attack_s3_s_game(fighter: &mut L2CAgentBase) {
    ...
    FT_MOTION_RATE(fighter, 2.0 / 5.0);
    frame(lua_state, 5.0);
...
unsafe fn metaknight_attack_hi3_game(fighter: &mut L2CAgentBase) {
    ...
    frame(lua_state, 7.0);
    if is_excute(fighter) {
        ATTACK(fighter, 0, 0, Hash40::new("top"), 7.0, 95, 120, 0, 40, 5.0, 0.0, 6.2, 5.7, Some(0.0), Some(6.2), Some(-4.5), 1.0, 1.0, *ATTACK_SETOFF_KIND_ON, *ATTACK_LR_CHECK_POS, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_sting"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_CUTUP, *ATTACK_REGION_SWORD);
    }    
    frame(lua_state, 8.0);
    if is_excute(fighter) {
