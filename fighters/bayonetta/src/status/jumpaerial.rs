...
unsafe fn jump_end(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[CURRENT_FRAME].get_i32() as f32 <= fighter.get_param_float("param_special_hi", "jump_count_reset_frame") {
        if [*FIGHTER_STATUS_KIND_SPECIAL_HI, *FIGHTER_STATUS_KIND_SPECIAL_S].contains(&fighter.global_table[STATUS_KIND].get_i32()) {
            ...
            PostureModule::set_pos(fighter.module_accessor, &Vector3f{x: PostureModule::pos_x(fighter.module_accessor), y: PostureModule::pos_y(fighter.module_accessor) - 6.0, z: PostureModule::pos_z(fighter.module_accessor)});
        }
