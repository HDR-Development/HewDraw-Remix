...
unsafe extern "C" fn sound_damageflyhi(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if VarModule::is_flag(agent.battle_object, vars::common::instance::IS_KILLING_BLOW) {
            PLAY_SE(agent, Hash40::new("vc_bayonetta_damagefly03"));
        } else {
...
unsafe extern "C" fn sound_dash(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(agent) {
        let dash_sfx_handle = SoundModule::play_se(boma, Hash40::new("se_bayonetta_dash_start"), true, false, false, false, app::enSEType(0));
...
unsafe extern "C" fn game_appeallwl(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        StatusModule::change_status_request(boma, *FIGHTER_STATUS_KIND_SQUAT_WAIT, false);
    }
}
...
pub fn install(agent: &mut Agent) {
    ...
    agent.acmd("sound_damageflytop", sound_damageflyhi, Priority::Low);
    ...
    agent.acmd("game_appealsl", game_appealsr, Priority::Low);
    agent.acmd("game_appealhir", game_appealhir, Priority::Low);
    agent.acmd("game_appealhil", game_appealhir, Priority::Low);
    agent.acmd("sound_appealhir", sound_appealhil, Priority::Low);
