...
unsafe extern "C" fn game_shot(agent: &mut L2CAgentBase) {
    ...
    if !palutena {
        if is_excute(agent) {
            ...
            if VarModule::get_int(owner_module_accessor.object(), vars::palutena::instance::CYAN_ENERGY) < 7 {
                MeterModule::drain_direct(owner_module_accessor.object(), 1.0);
            }
            VarModule::dec_int(owner_module_accessor.object(), vars::palutena::instance::CYAN_ENERGY);
        }
    } else {
        if is_excute(agent) {
