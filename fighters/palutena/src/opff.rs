    ...
    if fighter.is_status(*FIGHTER_PALUTENA_STATUS_KIND_SPECIAL_HI_3) {
        if StatusModule::is_changing(boma) {
            if fighter.is_situation(*SITUATION_KIND_GROUND) {
                ...
                VarModule::off_flag(boma.object(), vars::palutena::status::SPECIAL_HI_TELEPORT_AIR_START);
                if boma.get_num_used_jumps() < boma.get_jump_count_max()
                && !VarModule::is_flag(fighter.battle_object, vars::palutena::instance::SPECIAL_HI_ENABLE_FREEFALL) {
                    VarModule::on_flag(boma.object(), vars::common::instance::UP_SPECIAL_CANCEL);
                    VarModule::on_flag(boma.object(), vars::palutena::status::SPECIAL_HI_TELEPORT_AIR_START);
