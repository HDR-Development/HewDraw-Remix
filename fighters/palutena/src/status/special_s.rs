...
unsafe extern "C" fn special_s_init(fighter: &mut L2CFighterCommon) -> L2CValue {
    special_s_momentum_helper(fighter, true.into());
    ...
    if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_PALUTENA_INSTANCE_WORK_ID_FLAG_SPECIAL_N_LANDING) {
        let special_n_speed_y_mul = WorkModule::get_param_float(fighter.module_accessor, hash40("param_special_s"), hash40("special_s_speed_y_mul"));
...
unsafe extern "C" fn special_s_momentum_helper(fighter: &mut L2CFighterCommon, start: L2CValue) {
    ...
    if start.get_bool() {
        let special_n_speed_x_mul = WorkModule::get_param_float(fighter.module_accessor, hash40("param_special_s"), hash40("special_n_speed_x_mul"));
...
unsafe extern "C" fn special_s_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    && StatusModule::is_situation_changed(fighter.module_accessor) {
        special_s_momentum_helper(fighter, false.into());
