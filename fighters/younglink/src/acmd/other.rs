...
#[acmd_script( agent = "younglink", script = "sound_damageflyhi" , category = ACMD_SOUND , low_priority)]
...
unsafe fn younglink_appeal_lw_r_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
...
unsafe fn younglink_appeal_lw_l_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    frame(lua_state, 40.0);
    if is_excute(fighter) {
...
#[acmd_script( agent = "younglink", script = "sound_dash" , category = ACMD_SOUND , low_priority)]
...
#[acmd_script( agent = "younglink", script = "game_escapeair" , category = ACMD_GAME , low_priority)]
...
unsafe fn younglink_boomerang_game_fly(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        ...
        ATK_SET_SHIELD_SETOFF_MUL(fighter, 0,0.25);
    }
    wait(lua_state, 3.0);
    if is_excute(fighter) {
