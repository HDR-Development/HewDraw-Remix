...
unsafe extern "C" fn game_catchdash(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 9.0);
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 4.0, 0.0, 6.6, 6.0, Some(0.0), Some(6.6), Some(10.2), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    game_CaptureCutCommon(agent);
    wait(lua_state, 2.0);
    if is_excute(agent) {
...
unsafe extern "C" fn game_catchturn(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 4.0, 0.0, 6.6, -6.0, Some(0.0), Some(6.6), Some(-12.1), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    game_CaptureCutCommon(agent);
    wait(lua_state, 2.0);
    if is_excute(agent) {
...
unsafe extern "C" fn game_throwf(agent: &mut L2CAgentBase) {
    ...
    if !smash2::app::FighterCutInManager::is_vr_mode() {
        if smash2::app::FighterCutInManager::is_one_on_one_including_thrown(&*(agent.module_accessor as *const smash2::app::BattleObjectModuleAccessor)) {
            ...
            let lockout = VarModule::get_int(agent.battle_object, vars::demon::instance::CAMERA_LOCKOUT_TIMER);
            if lockout <= 0 {
                if WorkModule::is_flag(boma, *FIGHTER_DEMON_INSTANCE_WORK_ID_FLAG_ENABLE_THROW_MOTION_CAMERA) {
                    ...
                    VarModule::set_int(agent.battle_object, vars::demon::instance::CAMERA_LOCKOUT_TIMER, 180);
                }
            }
        }
    }
...
unsafe extern "C" fn game_throwcommand(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        REVERSE_LR(agent);
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 1.0, 155, 205, 0, 45, 0.0, 1.0, *ATTACK_LR_CHECK_B, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
...
pub fn install(agent: &mut Agent) {
    ...
    agent.acmd("game_throwcommand", game_throwcommand, Priority::Low);
}
