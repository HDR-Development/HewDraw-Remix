...
unsafe extern "C" fn demon_catch_game(fighter: &mut L2CAgentBase) {
...
unsafe extern "C" fn demon_catch_dash_game(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn demon_catch_turn_game(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.boma();
    frame(lua_state, 9.0);
    if is_excute(agent) {
...
pub fn install() {
    ...
    smashline::Agent::new("demon")
        .acmd("game_catch", demon_catch_game)
        .acmd("game_catchdash", demon_catch_dash_game)
        .acmd("game_catchturn", demon_catch_turn_game)
