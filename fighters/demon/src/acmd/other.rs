...
unsafe extern "C" fn dash_sound(fighter: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(fighter) {
        let dash_sfx_handle = SoundModule::play_se(fighter.module_accessor, Hash40::new("se_demon_dash_start"), true, false, false, false, app::enSEType(0));
...
unsafe extern "C" fn demon_appeal_s_r_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    frame(lua_state, 1.0);
    FT_MOTION_RATE_RANGE(fighter, 1.0, 18.0, 11.0);
    if is_excute(fighter) {
...
unsafe extern "C" fn demon_appeal_s_l_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    frame(lua_state, 1.0);
    FT_MOTION_RATE_RANGE(fighter, 1.0, 18.0, 11.0);
...
pub fn install() {
    smashline::Agent::new("demon")
        ...
        .acmd("game_appealsr", demon_appeal_s_r_game)
        .acmd("game_appealsl", demon_appeal_s_l_game)
        .install();
}
