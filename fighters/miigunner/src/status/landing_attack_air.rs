...
unsafe extern "C" fn landing_attack_air_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
    if fighter.is_motion(Hash40::new("attack_air_hi_boost")) {
...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, *FIGHTER_STATUS_KIND_LANDING_ATTACK_AIR, landing_attack_air_pre);
}