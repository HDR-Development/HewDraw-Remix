...
unsafe extern "C" fn game_fly(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 5.0);
    if is_excute(agent) {
        ModelModule::set_scale(boma, 0.8);
    }
    frame(lua_state, 6.0);
    if is_excute(agent) {
        ModelModule::set_scale(boma, 0.6);
    }
    frame(lua_state, 7.0);
    if is_excute(agent) {
...
unsafe extern "C" fn game_flythrowhi(agent: &mut L2CAgentBase) {
    ...
    let boma = agent.boma();
    if is_excute(agent) {
        ATTACK(agent, 0, 0, Hash40::new("top"), 3.0, 90, 140, 0, 66, 6.5, 0.0, 0.0, 0.8, Some(0.0), Some(0.0), Some(10.0), 1.5, 1.0, *ATTACK_SETOFF_KIND_OFF, *ATTACK_LR_CHECK_SPEED, false, 0, 0.0, 0, true, true, false, false, false, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_MIIGUNNER_BLASTER, *ATTACK_REGION_ENERGY);
    }
}
