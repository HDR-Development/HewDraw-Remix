...
unsafe extern "C" fn special_lw_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !fighter.is_situation(*SITUATION_KIND_GROUND) {
        fighter.set_status_kind_interrupt(statuses::peach::SPECIAL_AIR_LW);
        return 1.into()
    }
        ...
        if StatusModule::is_situation_changed(fighter.module_accessor) {
            ...
            if !fighter.is_situation(*SITUATION_KIND_GROUND) {
                ...
                if MotionModule::frame(fighter.module_accessor) <= 10.0 {
                    fighter.change_status(statuses::peach::SPECIAL_AIR_LW.into(), false.into());
                    return 1.into();
                }
...
unsafe extern "C" fn special_lw_end(fighter: &mut L2CFighterCommon) -> L2CValue {
    if ItemModule::is_have_item(fighter.module_accessor, 0) {
        ...
        && fighter.global_table[globals::STATUS_KIND] != statuses::peach::SPECIAL_AIR_LW
...
pub fn install(agent: &mut Agent) {
    ...
    agent.status(Pre, statuses::peach::SPECIAL_AIR_LW, special_air_lw_pre);
    agent.status(Main, statuses::peach::SPECIAL_AIR_LW, special_air_lw_main);
    agent.status(End, statuses::peach::SPECIAL_AIR_LW, special_air_lw_end);
