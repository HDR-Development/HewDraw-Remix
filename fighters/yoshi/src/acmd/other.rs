...
unsafe extern "C" fn sound_damagefly(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if VarModule::is_flag(agent.battle_object, vars::common::instance::IS_KILLING_BLOW) {
            PLAY_SE(agent, Hash40::new("vc_yoshi_damagefly02"));
        } else {
...
unsafe extern "C" fn sound_dash(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(agent) {
        let dash_sfx_handle = SoundModule::play_se(boma, Hash40::new("se_yoshi_dash_start"), true, false, false, false, app::enSEType(0));
...
pub fn install(agent: &mut Agent) {
    ...
    agent.acmd("sound_guardoff", sound_guardoff, Priority::Low);
    agent.acmd("expression_justshieldoff", expression_justshieldoff, Priority::Low);
}
