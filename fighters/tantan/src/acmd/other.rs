...
unsafe extern "C" fn sound_damageflyhi(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if !StopModule::is_stop(boma) {
            ...
            if play_vc == 0 {PLAY_FLY_VOICE(agent, Hash40::new("seq_tantan_rnd_futtobi01"), Hash40::new("seq_tantan_rnd_futtobi02"));}
        }
    }
    frame(lua_state, 1.1);
    if is_excute(agent) {
...
pub fn install(agent: &mut Agent) {
    ...
    agent.acmd("game_attackshortendrb3", game_attackshortendr1);
    agent.acmd("game_attacklongendr1", game_attackshortendr1);
    agent.acmd("game_attacklongendrb1", game_attackshortendr1);
    agent.acmd("game_attacklongendrb3", game_attackshortendr1);
}
