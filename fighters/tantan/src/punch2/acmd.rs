...
unsafe extern "C" fn game_attackshort(agent: &mut L2CAgentBase) {
    ...
    let clearFrame = if !WorkModule::is_flag(boma, *WEAPON_TANTAN_PUNCH1_INSTANCE_WORK_ID_FLAG_AIR) {17.0} else {15.0};
...
unsafe extern "C" fn game_attacklong(agent: &mut L2CAgentBase) {
    ...
    let clearFrame = if !WorkModule::is_flag(boma, *WEAPON_TANTAN_PUNCH1_INSTANCE_WORK_ID_FLAG_AIR) { 21.0 } else { 19.0 };
    frame(lua_state, clearFrame);
    if is_excute(agent) {
        AttackModule::clear_all(boma);
    }
...
unsafe extern "C" fn game_attacklonghold(agent: &mut L2CAgentBase) {
    ...
    let clearFrame = if !WorkModule::is_flag(boma, *WEAPON_TANTAN_PUNCH1_INSTANCE_WORK_ID_FLAG_AIR) { 21.0 } else { 18.0 };
    frame(lua_state, clearFrame);
    if is_excute(agent) {
        AttackModule::clear_all(boma);
    }
