...
unsafe fn damageflyhi_sound(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        if !StopModule::is_stop(fighter.module_accessor) {
            ...
            if rng == 0 {PLAY_FLY_VOICE(fighter, Hash40::new("seq_duckhunt_rnd_futtobi01"), Hash40::new("seq_duckhunt_rnd_futtobi02"));}
        }
    }
    frame(lua_state, 1.1);
    if is_excute(fighter) {
