...
unsafe extern "C" fn gamewatch_attack_s3_s_game(fighter: &mut L2CAgentBase) {
...
unsafe extern "C" fn gamewatch_attack_hi3_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    frame(lua_state, 1.0);
    ...
    frame(lua_state, 10.0);
    FT_MOTION_RATE_RANGE(fighter, 10.0, 16.0, 3.0);
    if is_excute(fighter) {
    ...
    FT_MOTION_RATE_RANGE(fighter, 16.0, 20.0, 1.0);
    if is_excute(fighter) {
    ...
    FT_MOTION_RATE_RANGE(fighter, 20.0, 21.0, 3.0);
    if is_excute(fighter) {
...
unsafe extern "C" fn gamewatch_attack_lw3_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    if is_excute(fighter) {
    ...
    frame(lua_state, 6.0);
    FT_MOTION_RATE_RANGE(fighter, 6.0, 11.0, 4.0);
    ...
    FT_MOTION_RATE_RANGE(fighter, 11.0, 16.0, 4.0);
    ArticleModule::set_rate(boma, *FIGHTER_GAMEWATCH_GENERATE_ARTICLE_NORMAL_WEAPON, (16.0 - 11.0)/4.0);
    if is_excute(fighter) {
        ATTACK(fighter, 0, 0, Hash40::new("top"), 9.0, 90, 100, 0, 36, 3.0, 0.0, 2.0, 8.7, Some(0.0), Some(4.3), Some(19.0), 1.0, 1.0, *ATTACK_SETOFF_KIND_ON, *ATTACK_LR_CHECK_POS, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_KICK, *ATTACK_REGION_OBJECT);
        AttackModule::set_attack_height_all(boma, app::AttackHeight(*ATTACK_HEIGHT_LOW), false);
    }
    frame(lua_state, 16.0);
    FT_MOTION_RATE(fighter, 1.0);
...
pub fn install() {
    smashline::Agent::new("gamewatch")
        .acmd("game_attacks3", gamewatch_attack_s3_s_game)
        .acmd("game_attackhi3", gamewatch_attack_hi3_game)
        .acmd("game_attacklw3", gamewatch_attack_lw3_game)
        .install();
}
