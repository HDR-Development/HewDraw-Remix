...
unsafe extern "C" fn special_hi_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.battle_object, vars::gamewatch::instance::SPECIAL_HI_FREEFALL) {
...
unsafe fn special_hi_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if MotionModule::is_end(fighter.module_accessor) {
        ...
        if VarModule::is_flag(fighter.battle_object, vars::gamewatch::instance::SPECIAL_HI_FREEFALL) {
            ...
            VarModule::on_flag(fighter.battle_object, vars::gamewatch::instance::SPECIAL_HI_PARACHUTE);
            fighter.change_status(FIGHTER_STATUS_KIND_FALL.into(), false.into());
        }
        return 1.into()
    }
    if fighter.status_frame() > 31 && fighter.is_cat_flag(Cat1::SpecialAny)
    && !VarModule::is_flag(fighter.battle_object, vars::gamewatch::instance::SPECIAL_HI_FREEFALL) {
        VarModule::on_flag(fighter.battle_object, vars::gamewatch::instance::SPECIAL_HI_PARACHUTE);
...
unsafe extern "C" fn special_hi_exit(fighter: &mut L2CFighterCommon) -> L2CValue {
    VarModule::on_flag(fighter.battle_object, vars::gamewatch::instance::SPECIAL_HI_FREEFALL);
    0.into()
}
