...
unsafe extern "C" fn game_attackhi3(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        FT_MOTION_RATE(agent, 1.0);
        ATTACK(agent, 0, 0, Hash40::new("top"), 17.0, 85, 73, 0, 84, 3.5, 0.0, 6.0, 8.0, None, None, None, 1.5, 1.0, *ATTACK_SETOFF_KIND_OFF, *ATTACK_LR_CHECK_POS, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_L, *COLLISION_SOUND_ATTR_HEAVY, *ATTACK_REGION_PUNCH);
    }
    frame(lua_state, 7.0);
    if is_excute(agent) {
...
pub fn install(agent: &mut Agent) {
    agent.acmd("game_attacks3melee", game_attacks3melee);
    agent.acmd("effect_attacks3melee", effect_attacks3melee);
    agent.acmd("sound_attacks3melee", sound_attacks3melee);;
    agent.acmd("expression_attacks3melee", expression_attacks3melee);
