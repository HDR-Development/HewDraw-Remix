...
unsafe extern "C" fn fgc_dashback_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let is_backdash_input: bool = (fighter.global_table[CMD_CAT1].get_i32() & *FIGHTER_PAD_CMD_CAT1_FLAG_TURN_DASH != 0) || (enable_bidou && ControlModule::check_button_trigger(fighter.module_accessor, *CONTROL_PAD_BUTTON_CSTICK_ON) && stick_x * -1.0 * PostureModule::lr(fighter.module_accessor) > 0.6);
    ...
    if !is_dash_input
    && !is_backdash_input
    && fighter.global_table[STICK_X].get_f32() * -1.0 * PostureModule::lr(fighter.module_accessor) < 0.0
...
pub unsafe fn fgc_end_dashback(fighter: &mut L2CFighterCommon) {
	...
	else if ![*FIGHTER_STATUS_KIND_DASH, *FIGHTER_STATUS_KIND_TURN_DASH].contains(&StatusModule::status_kind_next(fighter.module_accessor))
    && VarModule::is_flag(fighter.battle_object, vars::common::IS_MOONWALK) {
