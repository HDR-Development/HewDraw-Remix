...
const ZERO_VEC: Vector3f = Vector3f { x: 0.0, y: 0.0, z: 0.0 };

#[hook(module = "common", symbol = "_ZN7lua2cpp16L2CFighterCommon19sub_GuardDamageUniqEN3lib8L2CValueE")]
...
#[hook(module = "common", symbol = "_ZN7lua2cpp16L2CFighterCommon25status_GuardDamage_commonEN3lib8L2CValueE")]
unsafe fn status_GuardDamage_common(fighter: &mut L2CFighterCommon, arg: L2CValue) {
    ...
    if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_STATUS_GUARD_ON_WORK_FLAG_JUST_SHIELD) {
        ...
        if arg.get_bool() {
            ...
            let curr_shield = WorkModule::get_float(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_FLOAT_GUARD_SHIELD);
            ...
            let color = app::FighterUtil::get_effect_team_color(app::EColorKind(team_color as i32), Hash40::new("shield_effect_color"));
...
#[hook(module = "common", symbol = "_ZN7lua2cpp16L2CFighterCommon35status_guard_damage_main_common_airEv")]
unsafe fn status_guard_damage_main_common_air(fighter: &mut L2CFighterCommon) -> L2CValue {
    if fighter.global_table[SITUATION_KIND] == SITUATION_KIND_AIR {
        fighter.change_status(
            FIGHTER_STATUS_KIND_DAMAGE_FALL.into(),
            false.into()
        );
        true.into()
    } else {
...
#[hook(module = "common", symbol = "_ZN7lua2cpp16L2CFighterCommon31status_guard_damage_main_commonEv")]
unsafe fn status_guard_damage_main_common(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_enable_transition_term(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_GUARD) 
        && WorkModule::get_int(fighter.module_accessor, *FIGHTER_STATUS_GUARD_DAMAGE_WORK_INT_STIFF_FRAME) == 0 {
        fighter.change_status(
            FIGHTER_STATUS_KIND_GUARD.into(),
            false.into()
        );
        return true.into();
    }

    if WorkModule::get_int(fighter.module_accessor, *FIGHTER_STATUS_GUARD_DAMAGE_WORK_INT_STIFF_FRAME) == 0 {
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_STATUS_GUARD_ON_WORK_FLAG_JUST_SHIELD) {
    ...
    let damage = WorkModule::get_int(fighter.module_accessor, *FIGHTER_STATUS_GUARD_DAMAGE_WORK_INT_DAMAGE);
    let setoff_escape = WorkModule::get_param_int(fighter.module_accessor, hash40("common"), hash40("shield_setoff_escape"));
    let cat2 = fighter.global_table[CMD_CAT2].get_i32();

    if setoff_escape <= damage 
        && fighter.global_table[SITUATION_KIND] == SITUATION_KIND_GROUND
...
#[hook(module = "common", symbol = "_ZN7lua2cpp16L2CFighterCommon23status_GuardDamage_MainEv")]
...
#[common_status_script(status = FIGHTER_STATUS_KIND_GUARD_DAMAGE, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN,
    symbol = "_ZN7lua2cpp16L2CFighterCommon18status_GuardDamageEv")]
