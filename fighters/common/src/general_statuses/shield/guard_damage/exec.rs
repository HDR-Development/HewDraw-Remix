...
#[hook(module = "common", symbol = "_ZN7lua2cpp16L2CFighterCommon52sub_ftStatusUniqProcessGuardDamage_execStatus_commonEv")]
unsafe fn sub_ftStatusUniqProcessGuardDamage_execStatus_common(fighter: &mut L2CFighterCommon) {
    if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_STATUS_GUARD_ON_WORK_FLAG_JUST_SHIELD) {
        ...
        let shield_level = WorkModule::get_float(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_FLOAT_GUARD_SHIELD);
...
#[common_status_script(status = FIGHTER_STATUS_KIND_GUARD_DAMAGE, condition = LUA_SCRIPT_STATUS_FUNC_EXEC_STATUS,
    symbol = "_ZN7lua2cpp16L2CFighterCommon45sub_ftStatusUniqProcessGuardDamage_execStatusEv")]
...
#[hook(module = "common", symbol = "_ZN7lua2cpp16L2CFighterCommon50sub_ftStatusUniqProcessGuardDamage_execStop_commonEv")]
unsafe fn sub_ftStatusUniqProcessGuardDamage_execStop_common(fighter: &mut L2CFighterCommon) {
    ...
    if WorkModule::count_down_int(fighter.module_accessor, *FIGHTER_STATUS_GUARD_DAMAGE_WORK_INT_PREV_SHIELD_SCALE_FRAME, 0) != 0 {
        let handle = WorkModule::get_int(fighter.module_accessor, *FIGHTER_STATUS_GUARD_ON_WORK_INT_SHIELD_DAMAGE_EFFECT_HANDLE);
        if handle != 0 {
            EffectModule::set_scale(fighter.module_accessor, handle as u32, &Vector3f{ x: 0.1, y: 0.1, z: 0.1 });
        }

        let shield_level = WorkModule::get_float(fighter.module_accessor, *FIGHTER_STATUS_GUARD_DAMAGE_WORK_FLOAT_PREV_SHIELD);
        let scale = fighter_status_guard::calc_shield_scale(fighter, shield_level.into()).get_f32();
        ModelModule::set_joint_scale(fighter.module_accessor, Hash40::new("throw"), &Vector3f{ x: scale, y: scale, z: scale });
        ...
        WorkModule::set_int(fighter.module_accessor, frame, *FIGHTER_STATUS_GUARD_ON_WORK_INT_SCALE_FADE_START_FRAME);
    }

    let scale = if 0 < WorkModule::get_int(fighter.module_accessor, *FIGHTER_STATUS_GUARD_DAMAGE_WORK_INT_PREV_SHIELD_SCALE_FRAME) {
        let shield_level = WorkModule::get_float(fighter.module_accessor, *FIGHTER_STATUS_GUARD_DAMAGE_WORK_FLOAT_PREV_SHIELD);
        fighter_status_guard::calc_shield_scale(fighter, shield_level.into()).get_f32()
    } else {
        let shield_level = WorkModule::get_float(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_FLOAT_GUARD_SHIELD);
        let scale = fighter_status_guard::calc_shield_scale(fighter, shield_level.into()).get_f32();
        let handle = WorkModule::get_int(fighter.module_accessor, *FIGHTER_STATUS_GUARD_ON_WORK_INT_SHIELD_DAMAGE2_EFFECT_HANDLE);
        if handle != 0 {
            let size = if 0 < WorkModule::get_int(fighter.module_accessor, *FIGHTER_STATUS_GUARD_ON_WORK_INT_SCALE_FADE_START_FRAME) {
                ...
                let fade_frame = WorkModule::get_int(fighter.module_accessor, *FIGHTER_STATUS_GUARD_ON_WORK_INT_SCALE_FADE_START_FRAME) as f32;
...
#[common_status_script(status = FIGHTER_STATUS_KIND_GUARD_DAMAGE, condition = LUA_SCRIPT_STATUS_FUNC_EXEC_STOP,
    symbol = "_ZN7lua2cpp16L2CFighterCommon43sub_ftStatusUniqProcessGuardDamage_execStopEv")]
...
#[common_status_script(status = FIGHTER_STATUS_KIND_GUARD_DAMAGE, condition = LUA_SCRIPT_STATUS_FUNC_LEAVE_STOP,
    symbol = "_ZN7lua2cpp16L2CFighterCommon46FighterStatusUniqProcessGuardDamage_leave_stopEN3lib8L2CValueES2_")]
unsafe fn FighterStatusUniqProcessGuardDamage_leave_stop(fighter: &mut L2CFighterCommon, arg1: L2CValue, arg2: L2CValue) -> L2CValue {
    if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_STATUS_GUARD_ON_WORK_FLAG_JUST_SHIELD) {
    ...
    effect!(fighter, MA_MSC_CMD_EFFECT_EFFECT_OFF_KIND, Hash40::new_raw(0x113434cb66), false, false);
    effect!(fighter, MA_MSC_CMD_EFFECT_EFFECT_OFF_KIND, Hash40::new_raw(0x12be304eab), false, false);
    effect!(fighter, MA_MSC_CMD_EFFECT_EFFECT_OFF_KIND, Hash40::new_raw(0x12c9377e3d), false, false);
    L2CValue::I32(0)
}
