...
unsafe extern "C" fn damageflyhi_sound(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if !StopModule::is_stop(agent.module_accessor) {
            ...
            if play_vc == 0 {PLAY_FLY_VOICE(agent, Hash40::new("seq_szerosuit_rnd_futtobi01"), Hash40::new("seq_szerosuit_rnd_futtobi02"));}
        }
    }
    frame(lua_state, 1.1);
    if is_excute(agent) {
...
unsafe extern "C" fn damageflyroll_sound(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if !StopModule::is_stop(agent.module_accessor) {
            PLAY_FLY_VOICE(agent, Hash40::new("seq_szerosuit_rnd_futtobi01"), Hash40::new("seq_szerosuit_rnd_futtobi02"));
        }
    }
    frame(lua_state, 1.1);
    if is_excute(agent) {
...
unsafe extern "C" fn dash_sound(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 4.0);
    if is_excute(agent) {
        let dash_sfx_handle = SoundModule::play_se(agent.module_accessor, Hash40::new("se_szerosuit_dash_start"), true, false, false, false, app::enSEType(0));
        SoundModule::set_se_vol(boma, dash_sfx_handle as i32, 0.5, 0);
    }
    wait(lua_state, 8.0);
    if is_excute(agent) {
        PLAY_STEP(agent, Hash40::new("se_szerosuit_step_right_l"));
    }
    wait(lua_state, 6.0);
    if is_excute(agent) {
        PLAY_STEP(agent, Hash40::new("se_szerosuit_step_left_l"));
    }
}
