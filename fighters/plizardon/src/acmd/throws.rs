...
unsafe extern "C" fn plizardon_catch_game(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        CATCH(fighter, 0, Hash40::new("top"), 5.6, 0.0, 8.0, 0.0, Some(0.0), Some(8.0), Some(16.9), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
...
unsafe extern "C" fn plizardon_catch_dash_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    ...
    if is_excute(fighter) {
        CATCH(fighter, 0, Hash40::new("top"), 5.6, 0.0, 6.0, 5.0, Some(0.0), Some(6.0), Some(15.9), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    game_CaptureCutCommon(fighter);
    wait(lua_state, 3.0);
    if is_excute(fighter) {
...
unsafe extern "C" fn plizardon_catch_turn_game(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = fighter.boma();
    frame(lua_state, 11.0);
    if is_excute(fighter) {
    ...
    if is_excute(fighter) {
        CATCH(fighter, 0, Hash40::new("top"), 5.6, 0.0, 8.0, -4.0, Some(0.0), Some(8.0), Some(-17.0), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    game_CaptureCutCommon(fighter);
    wait(lua_state, 3.0);
    if is_excute(fighter) {
...
pub fn install() {
    smashline::Agent::new("plizardon")
        .acmd("game_catch", plizardon_catch_game)
        .acmd("game_catchdash", plizardon_catch_dash_game)
        .acmd("game_catchturn", plizardon_catch_turn_game)
