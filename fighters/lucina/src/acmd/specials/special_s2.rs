...
unsafe extern "C" fn effect_specials2hi(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        EFFECT_FOLLOW(agent, Hash40::new("lucina_mc_2hi_hdr"), Hash40::new("top"), 0, 0, 0, 0, 0, 0, 1, true);
...
unsafe extern "C" fn effect_specials2lw(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        EFFECT_FOLLOW(agent, Hash40::new("lucina_mc_2lw_hdr"), Hash40::new("top"), 0, 0.6, 0.2, 0, 0, 0, 1, true);
        EffectModule::set_disable_render_offset_last(boma);
    }
    frame(lua_state, 7.0);
    if is_excute(agent) {
