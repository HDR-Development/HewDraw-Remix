...
unsafe extern "C" fn special_s2_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    WorkModule::off_flag(fighter.module_accessor, *FIGHTER_MARTH_STATUS_SPECIAL_S_FLAG_CONTINUE_MOT);
    lucina_specials_reset_helper(fighter);
    ControlModule::reset_trigger(fighter.module_accessor);
    WorkModule::set_int(
...
unsafe extern "C" fn special_s3_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    WorkModule::off_flag(fighter.module_accessor, *FIGHTER_MARTH_STATUS_SPECIAL_S_FLAG_CONTINUE_MOT);
    if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_MARTH_STATUS_SPECIAL_S_FLAG_INPUT_LW) {
        WorkModule::set_int64(
            fighter.module_accessor,
...
unsafe extern "C" fn special_s3_execstop(_fighter: &mut L2CFighterCommon) -> L2CValue {
    0.into()
}
...
pub fn install(agent: &mut Agent) {
    ...
    agent.status(ExecStop, *FIGHTER_MARTH_STATUS_KIND_SPECIAL_S3, special_s3_execstop);
}