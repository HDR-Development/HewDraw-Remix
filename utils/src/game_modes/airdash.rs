...
unsafe fn check_airdash(boma: &mut BattleObjectModuleAccessor) {
    if boma.is_status(*FIGHTER_STATUS_KIND_ESCAPE_AIR_SLIDE) && boma.motion_frame() > 5.0 {
        ...
        if speed_x > 0. {
            EffectModule::req_on_joint(boma, Hash40::new("sys_whirlwind_r"), Hash40::new("top"),
            ...
            EffectModule::req_on_joint(boma, Hash40::new("sys_whirlwind_l"), Hash40::new("top"),
            &pos, &rot, 0.75, &Vector3f{x:0.0, y:0.0, z:0.0}, &Vector3f{x:0.0, y:0.0, z:0.0}, false, 0, 0, 0);
        }
